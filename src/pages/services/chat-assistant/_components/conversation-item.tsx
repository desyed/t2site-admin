...
export default function ConversationItem({
  ...
  useEffect(() => {
    (async () => {
      await delay(300);
      if (navigation.state != 'loading' && conversationItem.unread && clicked) {
        setConversationItem({
          ...conversationItem,
          unread: false,
        });
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between">
            <span
              className={cn('text-sm font-medium truncate uppercase max-w-48 ')}
            >
            ...
            >
              {formatSmartTimestamp(conversationItem.latestMessage.createdAt)}
            </span>
          </div>
          ...
          {conversationItem.latestMessage.content.type === 'emojiOrSticker' && (
            <p
              className={cn(
                'text-xs truncate text-muted-foreground',
                conversationItem.unread && 'text-foreground font-semibold'
